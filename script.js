...
// ================== Funções para o Pop-up de Natal ==================

/**
 * Abre o pop-up festivo de Natal, a menos que já tenha sido fechado na sessão atual.
 */
function abrirPopupNatal() {
    // Adicione um período de tempo (por exemplo, 2 segundos) para garantir que a tela carregue primeiro, se necessário.
    setTimeout(() => {
        // Verifica se o pop-up já foi fechado na sessão para não incomodar novamente (opcional)
        // Isso impede que ele reapareça se o usuário recarregar a página na mesma aba.
        if (sessionStorage.getItem('popupNatalFechado') !== 'true') {
            const popup = document.getElementById("popupNatal");
            if (popup) {
                // Reutilizando a função abrirModal para transição
                abrirModal(popup); 
            }
        }
    }, 2000); // Atraso de 2 segundos. Ajuste conforme necessário.
}

/**
 * Fecha o pop-up festivo de Natal e define uma flag de sessão.
 */
function fecharPopupNatal() {
    const popup = document.getElementById("popupNatal");
    if (popup) {
        // Reutilizando a função fecharModal para transição
        fecharModal(popup);
        // Define uma flag na sessão para que não apareça novamente até que o navegador seja fechado
        sessionStorage.setItem('popupNatalFechado', 'true');
    }
}

// Vincula a função de fechar ao clique fora do modal (overlay)
document.addEventListener('click', function(event) {
    const popup = document.getElementById("popupNatal");
    // Verifica se o clique foi no overlay do modal e se o modal está aberto
    if (popup && !popup.classList.contains('hidden') && event.target === popup) {
        fecharPopupNatal();
    }
});

// A sua função principal de inicialização é 'carregarAgenda()', que já está no 'window.addEventListener("load", carregarAgenda)'.
// Para evitar conflito, usaremos um novo listener para o pop-up ou incluiremos a chamada dentro de 'carregarAgenda()'.
// Neste caso, vamos adicionar um novo listener 'load', mas com um timeout para garantir que o 'carregarAgenda' comece primeiro.

// Vincula a função de abrir o pop-up de Natal ao evento de carregamento da página
window.addEventListener('load', abrirPopupNatal); 

// Adicional: Também vincula o botão 'X' (fechar-btn) se o 'onclick' não estiver no HTML
const btnFecharPopup = document.querySelector('#popupNatal .fechar-btn');
if (btnFecharPopup) {
    btnFecharPopup.addEventListener('click', fecharPopupNatal);
}
